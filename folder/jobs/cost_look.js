...
Costs.lookUp=(indek,id_kolom)=>{
  bingkai[indek].id_kolom=id_kolom;
  objPop=new CostLook(indek);
  objPop.show();
}

function CostLook(indek_parent){
  
  function show(){
    const tiket=JSON.parse(JSON.stringify(bingkai[indek_parent]));
    tiket.parent=indek_parent;
    tiket.modul='cost_codes';
    tiket.menu.name="Cost Codes"
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "jobs":
        Jobs.setCost(indek_parent,data[a]);
        break;

      default:
        alert('['+bingkai[indek_parent].menu.id+']'
        +' undefined in [cost_look.js]');
    }
    ui.CLOSE_POP(indek);
  }
