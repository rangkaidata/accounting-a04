...
function CustomerLook(indek_parent,cari){
  function show(){
    const tiket=JSON.parse(JSON.stringify(bingkai[indek_parent]));
    tiket.parent=indek_parent;
    tiket.modul='customers';
    tiket.menu.name="Customers"
  ...
  function select(indek,a){
    ...
    switch(bingkai[indek_parent].menu.id){
      case "jobs":
        Jobs.setCustomer(indek_parent,data[a]);
        break;

      default:
        alert('['+bingkai[indek_parent].menu.id+']'
          +' undefined in [customer_look.js]');        
    }
    ui.CLOSE_POP(indek);
  }
