...
DiscountTerms.formEntry=(indek)=>{
  var html=''
    ...
    +'<li><label>Date:</label>'
      +'<input type="date" id="date_'+indek+'" disabled></li>'
      
    +'<li><label>Amount:</label>'
      +'<input type="text" id="amount_'+indek+'"'
      +' onfocus="this.select()" '
      +' size="9" style="text-align:center;" disabled>'
      +'</li>'
    ...
    +'<li><label>Net due in:</label>'
      ...
      +' onfocus="this.select()" '
      +' onchange="DiscountTerms.calc(\''+indek+'\')"></li>'
    ...
    +'<li><label>Date due:</label>'
      +'<input type="date" id="date_due_'+indek+'" disabled></li>'
      
    +'<li><label>Discount date:</label>'
      +'<input type="date" id="discount_date_'+indek+'" disabled></li>'

    +'<li><label>Discount amt:</label>'
      +'<input type="text" id="discount_amount_'+indek+'"'
      +' size="9" style="text-align:center;" disabled></li>'
      
    +'<li><label>Terms:</label>'
      +'<input type="text" id="displayed_'+indek+'"'
      +' size="15" style="text-align:left;"></li>'
...
DiscountTerms.setData=(indek)=>{
  ...
  const terms=bingkai[indek_parent].discount_terms;
...
DiscountTerms.calc=(indek)=>{
  const due_in=getEV('due_in_'+indek);
  const date=new Date(getEV('date_'+indek) );
  const discount_in=getEV('discount_in_'+indek);
  const discount_percent=getEV('discount_percent_'+indek);
  const amount=getEV('amount_'+indek);
  ...
  const nomer=document.getElementById('type_'+indek).selectedIndex;
  if(nomer==0 || nomer==1){
